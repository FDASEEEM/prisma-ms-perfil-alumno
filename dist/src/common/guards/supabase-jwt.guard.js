...
let SupabaseJwtGuard = class SupabaseJwtGuard {
    async canActivate(context) {
        ...
        try {
            const decoded = jsonwebtoken_1.default.verify(token, database_config_1.supabaseConfig.jwtSecret);
            if (!decoded.user_id) {
                throw new common_1.UnauthorizedException('Invalid token: missing user_id');
            }
            request['user'] = {
